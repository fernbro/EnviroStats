...
```{r}
annual_quakes <- subset(earthquakes_sf, earthquakes_sf$Magnitude >= 3) %>% 
...
```{r}
# had to just click on the .Rdata file from the file viewer to get it to load in... load() was not working as expected
...
plot(eq_f)
```
Based on our empirical G(r) and F(r) estimates, it would appear that our data are not arising from a HPP. G(r) shows that our points are more clustered than expected, and F(r) shows that there are more empty spaces than expected from a HPP.
...
plot(f_mod1)
```
These figures suggest that our fitted model predicts fewer close neighbors than were observed in the data, and that our model is overestimating the amount of proximate empty spaces. Our model is still probably underestimating the spatial clustering of these earthquakes.
...
summary(well_mod2)
```
Here, our confidence interval contains zero, indicating that there was no evidence for an effect of minimum distance after accounting for spatial dependence in the LGCP model.
